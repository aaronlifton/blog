...
describe("blog", () => {
  ...
  it("lists all posts with valid links", () => {
    ...
    cy.get(".post-preview")
      .should("have.length", 8).as('posts')
    cy.get("@posts").first().then(($post: HTMLElement) => {
      ...
      .then((posts) => {
        posts.each(
          (
            ...
            if (!posts[idx]) {
              console.error("Failed to fetch post data before running tests. Check CI logs to ensure the server can be reached");
            }
            const title = cy.wrap($post).find(".post-preview__title");
            title.find(">a").as("link");
            cy.get("@link")
    ...
    for (let idx = 0; idx < postUrls.length; idx++) {
      ...
      cy.get('@img').then($img => {
        ...
        middle = {x: w/2, y: h/2}
          ...
          cy.wait(200)
          cy.get('button img').first().click(middle.x, middle.y)
          ...
          cy.wait(100)
          cy.get('dialog button').first().should($btn => expect($btn).attr('aria-label').to.equal('Close'))
            .click()
          cy.wait(100)
          cy.get('dialog')
