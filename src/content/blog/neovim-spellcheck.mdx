...
import image2 from "./assets/vale-diagnostics-sample_qgdcfz_c_pad,h_300,e_sharpen.png";
...
1. Create a custom spelling suggestion file in your neovim configuration
   ...
   in that folder that end in `.add` — the words in these files are ignored by
   spellcheck.
...
3. Vale will automatically fetch the Style rules for you and save them into your
   `StylesPath`.
4. Set up a keybinging in Neovim to append `<cword>`, the word under the cursor,
   ...
   of words to ignore seperated by newlines.
