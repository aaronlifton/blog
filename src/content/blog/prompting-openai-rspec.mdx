...
request specs using `get ":create"` or `post ":update"` instead of the proper
routes, such as `api_v1_users_path` or `api_v1_user_path(user.id)`. It was
slightly tricky to generate a prompt with the exact wording so that OpenAI would
only use proper routes. To implement this, I was able to use the Neovim plugin
