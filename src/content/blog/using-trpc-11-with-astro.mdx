...
coverAlt:
  "An example of a tRPC router, taking from the source code of this site."
...
However, several RPC use HTTP as their protocol, such as GRPC, which is built
upon HTTP, so as to take advantage of the many great features of HTTP/2:
bidirectional streaming, such as in websockets, multiplexed requests (concurrent
server calls over a single connection), and header compression. RPC differs from
REST in that, the client only needs to call a server method in order to
communicate with the server. Behind the scenes, the server method is stubbed,
where serialization, communication, and deserialization take place, hidden from
the developer-facing API. This enables developers to focus on implementing API
functionality, such as business logic, rather than the details of the protocol
used. RPCs also benefit from type-safety - I had worked before in an environment
that used GRPC to automatically create generate client side APIs for multiple
languages, so that each team worked with the exact same set of type definitions.
...
1. First, we'll set up the ["context"](https://trpc.io/docs/server/context) for
   ...
   context is always available in your procedures, while outer context depends
   on the request. This is useful for authentication, because session
   information would be found in the request. This kind of context is only
   available for procedures called via HTTP, which makes sense.
...
Now that we have context and a server, the last piece on the server side will be
...
in both the client and server. Prisma supports upsort operations on most
database; in this example I was using a SQLite database provided by Turso.

```ts
interface IncrementMetricInput {
...
Now our RPC server has a single procedure, `getFile`. In this post, we will call
this procedure from the server-side. This is useful for micro-frontend
architectures, where the RPC server may live in a separate package. Calling the
RPC from the server side involves creating a `caller` via the
