...
prediction after this gets deployed:
...
const router = router({
  incrementMetric: publicProcedure
    .input(z.enum(["views", "gh_ratelimit_errors"]))
    .mutation((resolver) => {
...
`gh_ratelimit_errors` metrics can be incremented, similar to how one would use
StatsD metrics. And the best part? **The type restriction is enforced at compile
time.**
...
It took me some time to figure out how to test tRPC, but I'm adding this note to
this article to say that a new post covering how to test tRPC and Astro with
Vitest is coming soon. I've noticed that some companies on github don't mock
Prisma in the test environment, and now that I've tested tRPC with Vitest mocks,
I'm working on setting up a Rails-like test database setup.
