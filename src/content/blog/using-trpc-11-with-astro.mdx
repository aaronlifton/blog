...
pubDate: Mar 24 2024
cover: ./assets/screenshots/github-rest-api.jpg
coverAlt: ""
...
server may include editor support for code completion, and saves the developer
from having to deal with the implementation details of HTTP or REST. Instead of
making a POST or PUT request to the server, RPCs typically involves more direct
communication with the server. tRPC integrates with newer best practices like
using React Query to hydrate the client with data from the server, instead of
reimplementing the same logic in its own library. I had worked before in an
environment that used GRPC, and there the GRPC definitions generated Ruby,
Kotlin, and JS definitions so that each team worked with the exact same set of
type definitions.

For example, if we specified that on the RPC server, we require a
`{name: string}` parameter for the `createUser` procedure, the client would not
build without passing the correct typed parameter:
...
const router = router({
  create: publicProcedure
    .input(z.object({ name: z.string() }))  // [!code highlight]
    .mutation(async (opts) => {
      const { input } = opts;
      const user = await prisma.user.create({
        data: {
...
Today we'll build a React component that uses Suspense to render a loading state
while the backend streams data from the Github API, via a tRPC procedure. This
idea came from me wanting to display an auto-refreshing code snippets for other
articles. To accomplish this task, we need to set up tRPC and write a procedure
that fetches a file from a github repository.

## Installing tRPC v11

First, let's install v11 of tRPC.
[tRPC v11](https://trpc.io/docs/migrate-from-v10-to-v11) packages have the
`@next` release tag, and we can install them with the following command:
...
The tRPC team states that the `next` tag is
...
1. First, we'll set up the ["context"](https://trpc.io/docs/server/context) for
   ...
   Inner context provides context to all server-calls that don’t depend on a
   ...
   tRPC's where `req` and `res` are not available (See more about
   `createServerSideHelpers`). Inner context is always available in your
   procedures.

   Outer context depends on the request. This is useful for authentication,
   because session information would be found in the request. This kind of
   context is only available for procedures called via HTTP.
...
// Outer context. This is a superset of the inner context.
...
Now that we have context, and a server, the last piece on the server side will
be writing a procedure. Let's start by creating a new procedure call in a file
...
looks like so we know what to expect.
...
the rate limit. With the right combination of logic based on the headers, the rate limit should never be reached.
Let's add a metric so we can confirm our prediction after this gets deployed:

Let's create a new file in the `rpc-server` folder called `types.ts`:
```ts
import { IncrementMetricInput } from "./types";
```

Then, let's create a route in the tRPC router that will accept input of that
type, so we can take advantage of type safety in both the client and server.
```
const router = router({
	incrementMetric: publicProcedure.input(
		z.enum(["views", "gh_ratelimit_errors"])
	).mutation((resolver) => {
...
Nice! Now our API only allows incrementing of the `views` and `gh_ratelimit_errors` metrics can be incremented, similiar to how one would use StatsD metrics.
And the best part? **The type restriction is enforced at compile time.**
...
One nuanace of the Github API is that it returns an array of objects if the path
...
async (props: { owner: string; repo: string; path: string }) => {
  ...
  try {
    ...
    const rateLimitReset = body.headers["x-ratelimit-reset"];
    // handleRateLimit({ rateLimitRemaining, rateLimitReset });

    const data = body.data;
    if (Array.isArray(data)) {
      //                   ^
      //          [this scopes the type to the single object version]
...
Now that we've wrote a function that fetches a file from a certain path in a
...
Now our RPC server has a single procedure, `getFile`. Let's first call this
procedure from another server-side endpoint. This involves creating a `caller`
via the `createCallerFactory` function, as stated
[here](https://trpc.io/docs/server/server-side-calls). hooked everything up to

In my case, I'm using Astro.js, so I'll create a new endpoint in the `src/pages`
